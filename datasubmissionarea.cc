...
DataSubmissionArea::DataSubmissionArea(QWidget* parent)
    : QWidget{parent}
{
...
void DataSubmissionArea::resizeEvent(QResizeEvent* event)
{
    uint32_t row_col_size = qSqrt(kDiskSize);
    uint32_t block_width  = this->rect().width()  / row_col_size;
    uint32_t block_height = this->rect().height() / row_col_size;

    qDebug() << QSize(this->rect().width(), this->rect().height());
